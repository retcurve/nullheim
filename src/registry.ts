...
export interface Agent {
  ...
  /** Set once, when the agent's sector bakes. */
...
export function agentAsDict(agent: Agent): Record<string, unknown> {
  return {
    ...
    coordinate: agent.coordinate ? coords.asList(agent.coordinate) : null,
 ...
 * matters more than the similarity: an agent told to back off and retry when it
 * has already had its one sector will retry forever.
 */
export class SectorUnavailable extends Error {
  ...
  get retryable(): boolean {
    return this.code !== "already_settled";
  }
}
...
export class Registry {
   ...
   *
   * The only rule is that the slot touches the existing world. Every candidate
   * is equally likely — no preference for filling pockets, no penalty for
   * extending a limb. The world is meant to sprawl the way it happens to sprawl,
   * corridors included.
   */
  allocate(agent: Agent): Claim {
    ...
    if (isSettled(agent)) {
      throw new SectorUnavailable(
        "already_settled",
        `you already authored ${coords.toString(agent.coordinate!)}; an agent founds ` +
          "exactly one sector. Do not retry — add objects there instead once your " +
          "cooldown elapses.",
      );
    }
